...
pub struct NodeEditorState {
    ...
    pub zoom: f32,
    pub show_node_creation_menu: bool,
    pub node_creation_pos: Option<Vec2>,
}
...
impl NodeEditorState {
    ...
    pub fn handle_node_creation(&mut self, ui: &mut Ui, node_graph: &mut NodeGraph) -> bool {
        if !self.show_node_creation_menu {
            return false;
        }
                ...
                egui::ScrollArea::vertical().show(ui, |ui| {
                    for node_type in NodeType::all() {
                        if ui.button(node_type.name()).clicked() {
                            if let Some(pos) = self.node_creation_pos {
                                if let Some(node) = node_type.create() {
                                    let id = node_graph.add_node(node);
                                    self.set_node_position(id, pos);
                                    created = true;
                                    self.show_node_creation_menu = false;
                                }
                            }
                        }
                    }
...
pub fn render_node_editor(
    ...
    // Handle right-click for node creation
    if response.clicked_by(egui::PointerButton::Secondary) && !state.show_node_creation_menu {
        if let Some(pos) = response.hover_pos() {
            state.show_node_creation_menu = true;
            state.node_creation_pos = Some((pos - rect.min - state.scroll_offset) / state.zoom);
        }
    }
    ...
    ui.input(|i| {
        ...
        if i.key_pressed(egui::Key::Escape) {
            state.show_node_creation_menu = false;
        }
    });
    ...
    for (node_id, node, pos) in nodes_to_draw {
        ...
        // Convert Vec<(String, Rect)> to &[(String, Rect)]
        let slot_refs: Vec<(&str, egui::Rect)> = slot_rects.iter()
            .map(|(s, r)| (s.as_str(), *r))
            .collect();
        
        // Handle node interactions
        if state.handle_node_interaction(&response, &node_id, node_rect, &slot_refs) {
            response.mark_changed();
        }
    }

    // Draw and handle connections
    ...
    if state.handle_node_creation(ui, node_graph) {
        response.mark_changed();
    }
